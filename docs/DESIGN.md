...
> Design notes for **saas-agent-onboarding-failures** — a runnable python example that Track failed SaaS tenant onboarding transitions with a feature flag and captured exception.
...
- **Thin client** — a ~30-line helper that owns the base URL, the auth header, and envelope unwrapping, so call sites stay readable (e.g. `infrai.flags.is_enabled(...)`).
- **Feature code** — the actual task: agent-error-tracking.
